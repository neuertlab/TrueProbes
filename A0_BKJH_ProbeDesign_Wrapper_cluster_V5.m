...
addpath('./../modified_matlab_functions');
addpath('./../core');
addpath('./thirdparty');
...
designerName = '_NLPDS';
BLASTpath = strcat('src',filesep,'thirdparty',filesep,'blast-2.8.1+',filesep,'bin',filesep,'blastn');
Nmodel = 4;
...
if (strcmp(referenceType,'RefSeq'))
...
elseif (strcmp(referenceType,'ENSEMBL'))
...
settings.mLoc2 = '"data/DatabaseData/Blast_Databases/Mouse/EMBL_EBI/Mouse_ENSEMBL_transcript_PlusrRNA"';%ENMUST, cdna and ncRNA
...
elseif (strcmp(referenceType,'Gencode'))
...
elseif (settings.SingleOrMulti==0&&settings.AllIsoforms == 0)%Multi Gene/One Isoform
    ...
    for k = 1:size(inputs1{gene_num,1},2)
       try 
           ...
           probes = init_probes; 
           save([saveRoot filesep FoldName{k} filesep FoldName{k} '_probes' designerName '.mat'],'probes','-v7.3')
       end   
    ...
    for k = 1:size(inputs1{gene_num,1},2)
       for k2 = 1:length(Multi_ENST_RefSeqIDz{k})
            try 
                ...
                probes = init_probes; 
                save([saveRoot filesep FoldName{kc(k,k2)} filesep FoldName{kc(k,k2)} '_probes' designerName '.mat'],'probes','-v7.3') 
            end   
        ...
        for k = 1:size(ENST_RefSeqIDz,1)       
            try
                ...
                [Kb_mod,Kb_Complement,DoesProbeBindSite2,Num_of_Molecule_Sites,MolProbesAtEvents,Mol_ProbesAtEventsID,nascentInfo,...
                 dHeq_mod,dSeq_mod,dHf_mod,dSf_mod,dHr_mod,dSr_mod,Tm_mod,dCp_mod,...
                 dHeq_Complement,dSeq_Complement,dHf_Complement,dSf_Complement,dHr_Complement,dSr_Complement,dCp_Complement] = ...
                    A_JH_GetSiteMapping_V6(Isoform_probes{k},settings,Isoform_gene_table{k},Multi_Kb_Match{k},Multi_dHeq_Match{k},Multi_dSeq_Match{k},Multi_dHf_Match{k},Multi_dSf_Match{k},Multi_dHr_Match{k},Multi_dSr_Match{k},Multi_Tm_Match{k});
            end 
        ...
        for k = 1:size(inputs1{gene_num,1},2)        
            try
                ...
                [Kb_mod,Kb_Complement,DoesProbeBindSite2,Num_of_Molecule_Sites,MolProbesAtEvents,Mol_ProbesAtEventsID,nascentInfo,...
                 dHeq_mod,dSeq_mod,dHf_mod,dSf_mod,dHr_mod,dSr_mod,Tm_mod,dCp_mod,...
                 dHeq_Complement,dSeq_Complement,dHf_Complement,dSf_Complement,dHr_Complement,dSr_Complement,dCp_Complement] = ...
                    A_JH_GetSiteMapping_V6(Transcript_probes{k},settings,Transcript_gene_table{k},Multi_Kb_Match{k},Multi_dHeq_Match{k},Multi_dSeq_Match{k},Multi_dHf_Match{k},Multi_dSf_Match{k},Multi_dHr_Match{k},Multi_dSr_Match{k},Multi_Tm_Match{k});
            end
            Multi_DPS2{k} = DoesProbeBindSite2;
            Multi_KbMod{k} = Kb_mod;
            Multi_NMS{k} = Num_of_Molecule_Sites;
        ...
        for k = 1:size(inputs1{gene_num,1},2)       
            ...
            try
                ...
                [Kb_mod,Kb_Complement,DoesProbeBindSite2,Num_of_Molecule_Sites,MolProbesAtEvents,Mol_ProbesAtEventsID,nascentInfo,...
                 dHeq_mod,dSeq_mod,dHf_mod,dSf_mod,dHr_mod,dSr_mod,Tm_mod,dCp_mod,...
                 dHeq_Complement,dSeq_Complement,dHf_Complement,dSf_Complement,dHr_Complement,dSr_Complement,dCp_Complement] = ...
                A_JH_GetSiteMapping_V6(TranscriptIsoform_probes{kc(k,k2)},settings,TranscriptIsoform_gene_table{kc(k,k2)},Multi_Kb_Match{kc(k,k2)},Multi_dHeq_Match{kc(k,k2)},Multi_dSeq_Match{kc(k,k2)},Multi_dHf_Match{kc(k,k2)},Multi_dSf_Match{kc(k,k2)},Multi_dHr_Match{kc(k,k2)},Multi_dSr_Match{kc(k,k2)},Multi_Tm_Match{kc(k,k2)});
            end 
...
elseif (needToCombine)
 ...
 %3D vs 4D site 25046952-2*12614976  [less memory 1 4D vs 2 3D
...
%% Get Design Landscape Properties
try
