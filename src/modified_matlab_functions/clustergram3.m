...
%
%   See also DENDROGRAM, HEATMAP, LINKAGE, MAPCAPLOT,
%   PDIST, REDBLUECMAP, REDGREENCMAP.
...
properties(SetObservable=true, AbortSet=true)
    ...
    %    the available options. It can be a character vector or a
    %    single-element cell array of character vectors used for both rows
    %    and columns, or a two-element cell array of character vectors, the
    %    first element is used for the linkage between rows, and the second
    %    element is used for the linkage between columns. To specify a
    %    linkage method for only one dimension, set the element for the
    %    other dimension to ''. The default method is 'average' for both
    %    dimensions.  
    %
...
methods
    function obj = clustergram3(data, varargin)
        %CLUSTERGRAM Create a clustergram object.
        %
        ...
        %   the rows in the heat map. ROWLABELS can be a string vector, 
        %   cell array of character vectors or a numeric array. 
        %
        %   CLUSTERGRAM(...,'COLUMNLABELS',COLUMNLABELS) specifies the
        %   labels for the columns in the heat map. COLUMNLABELS can be a
        %   string vector, cell array of character vectors, or a numeric array. 
        %
...
%                 viewClustergram3(obj,  groupIdx, fDim, inPV.Color)
            end
...
%         viewClustergram3(obj);
    ...
    function updateDisplay(obj, src, evt) %#ok
        switch src.Name
            case 'ShowDendrogram'
%                 viewClustergram3(obj, obj.ShowDendrogram)
            otherwise
                if ~obj.CopyOnly
%                     viewClustergram3(obj);
                end
    ...
    function computeClusters(obj)
        ...
        [Z1, H1, ~, perm1,D1] = computeDendrogram(data,...
        ...
        if strcmpi(obj.Cluster, 'ALL')
            linkageArgs = getLinkageArgs(obj, 1);
            dendroArgs = getDendroArgs(obj, 1);
            [Z2, H2, ~, perm2,D2] = computeDendrogram(data(perm1, :)',...
                obj.OptimalLeafOrder,...
                'Top',...
                obj.ColumnPDist,...
                linkageArgs,...
    ...
    function newObj = createNewClustergramObj(obj)
        ...
        newObj = clustergram(obj.PreOrderOriginalData,...
                             'CopyOnly', true,...
    ...
    function positionAxes(obj, hHMAxes)
        ...
        % Re-align "LabelsWithMarkers" axes.
        if obj.LabelsWithMarkers && ~isempty(obj.ColumnLabelsColor)
            markerRatio = obj.AnnotMarkerRatio;
            aPos = [hmPos(1) hmPos(2) - hmPos(4)/markerRatio hmPos(3) hmPos(4)/markerRatio];
            obj.XLabeledAxes.Position = aPos;
        end
        
        if obj.LabelsWithMarkers && ~isempty(obj.RowLabelsColor)
            markerRatio = obj.AnnotMarkerRatio;
            aPos = [hmPos(1) + hmPos(3) hmPos(2) hmPos(3)/markerRatio hmPos(4)];
            obj.YLabeledAxes.Position = aPos;
        end
...
%--------------------
function [Z, lineH, T, Perm, dist] = computeDendrogram(data, leafOrderFlag,...
                                                 dendroLoc, pdistArgs,...
