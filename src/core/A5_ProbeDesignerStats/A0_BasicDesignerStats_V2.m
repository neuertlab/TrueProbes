...
if (isRNA)
    ...
    if (~isfile([settings.FolderRootName filesep TranscriptName '_' settings.rootName '_BasicStats_RNA_ByProbe_Info' settings.designerName '.mat']))%check if temp file exists
        ...
        parfor v = 1:l;ength(batch_nums_to_check)
            ...
            temp_designer_stat_rna_p{v}{2} = sum(cellfun(@length,Sx(p,Js_OFFRNA(p))));
            temp_designer_stat_rna_p{v}{3} = cell2mat(Sx(p,Js_OFFRNA(p)));
            temp_designer_stat_rna_p{v}{4} = cell2mat(arrayfun(@(x) repmat(Js_OFFRNAi(p,x),[1 length(Sx(p,Js_OFFRNAi(p,x)))]),1:length(Js_OFFRNA(p)),'Un',0));
            temp_designer_stat_rna_p{v}{5} = log10(diag(full(squeeze(Kb_List.Value(p,temp_designer_stat_rna_p{v}{4},temp_designer_stat_rna_p{v}{3}))))');
            temp_designer_stat_rna_p{v}{6} = log10(diag(full(squeeze(Kb_List.Value(p,temp_designer_stat_rna_p{v}{4},temp_designer_stat_rna_p{v}{3}))))'/Kon_List.Value(p));
            temp_designer_stat_rna_p{v}{7} = log10(Kon_List.Value(p)./diag(full(squeeze(Kb_List.Value(p,temp_designer_stat_rna_p{v}{4},temp_designer_stat_rna_p{v}{3}))))');
            parsave_partial_on_off([FolderRootName filesep TranscriptName designerName '_basicStatsRNA_Info_probebatch' num2str(p) '.mat'],temp_on_off{v});
            temp_on_off{v} = [];
        end
        for p = 1:size(DoesProbeBindSite,1)
            Nvec_RNAsingle(p) = temp_designer_stat_rna_p{1};
            Nvec_RNAmulti(p) = temp_designer_stat_rna_p{2};
            Svec_RNA{p} = temp_designer_stat_rna_p{3};
            Tvec_RNA{p} = temp_designer_stat_rna_p{4};
            Tvec_logKOFF_RNA{p} = temp_designer_stat_rna_p{5};
