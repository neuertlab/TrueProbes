import sys, argparse
# This is the function for running blast local using Python for genomic DNA
...
#cline = NcbiblastnCommandline(db=database,query="blast_example_oneseq.fasta",out='results.xml',outfmt=5)
#cline = NcbiblastnCommandline(db=database,query=sequence_data,out=outputfile,outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_descriptions=1000,num_alignments=1000,dust='no',evalue=1000)
#cline = NcbiblastnCommandline(db='"' + database + '"',query=sequence_data,out=outputfile,outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_descriptions=1000,num_alignments=3,dust='no',evalue=1000)
#cline = NcbiblastnCommandline(db='"' + database + '"',query='"' + sequence_data + '"',out='"' + outputfile + '"',outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_descriptions=1000,num_alignments=1000,dust='no',evalue=1000)
#cline = NcbiblastnCommandline(db='"' + database + '"',query='"' + sequence_data + '"',out='"' + outputfile + '"',outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_alignments=1000,dust='no',evalue=1000)
#cline = NcbiblastnCommandline(db='"' + database + '"',query=sequence_data,out=outputfile,outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_descriptions=1000,num_alignments=3,dust='no',evalue=1000)
#Wthout quotes
#Limit output alignment percentage (perc_identity)
per_id = 100*mhs/pl
...
num_alignments = 1000
dust = 'no'
sub_command = location + ' -out ' + outputfile + ' -outfmt ' + str(outfmt) + ' -num_alignments ' + str(num_alignments) + ' -query ' + sequence_data + ' -db ' + database + ' -evalue ' + str(evalue) + ' -word_size ' + str(word_size) + ' -gapopen ' + str(gapopen) + ' -gapextend ' + str(gapextend) + ' -penalty ' + str(penalty) + ' -reward ' + str(reward) + ' -dust '+ dust + ' -perc_identity ' + str(per_id)
print(sub_command)
...
#cline = NcbiblastnCommandline(cmd=location,db=database,query=sequence_data,out=outputfile,outfmt=5,reward=1,penalty=-3,word_size=7,gapopen=5,gapextend=2,num_alignments=1000,dust='no',evalue=1000,perc_identity = per_id)
#print(cline)
#tdout, stderr = cline()
