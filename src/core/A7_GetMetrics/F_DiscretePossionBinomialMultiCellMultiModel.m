...
if (sum(isinf(q0)+isnan(q0),'all')+sum(alphas==0,'all')==0)
    ...
    if ~(ismcc || isdeployed)
        ...
        alphas_hpf = permute(repmat(CATnWrapper(arrayfun(@(cell) CATnWrapper(arrayfun(@(model) prod(hpf(full(squeeze(p1(:,:,model,cell)))),2),1:n_models,'Un',0),2),1:n_cells,'Un',0),3),[1 1 1 n_sites+1]),[1 4 2 3]);%is alpha needed if i just normalize it in the end
        q0s = CATnWrapper(arrayfun(@(C) CATnWrapper(arrayfun(@(C) CATnWrapper(arrayfun(@(C) [1 zeros(1,size(e,1))]',1:size(s,3),'Un',0),2),1:size(s,4),'Un',0),3),1:size(s,1),'Un',0),4);%[Sites Models Cell Target]
        q0_hpf = hpf(q0s);
        e_hpf = hpf(e);
        e1_hpf = hpf(e1);
        for j=1:n_sites
            q0_hpf(2:(j+1),:,:,:) = q0_hpf(2:(j+1),:,:) - repmat(e_hpf(j,:,:),[j 1 1 1]).*q0_hpf(1:j,:,:,:);
        end
        q_hpf = permute(CATnWrapper(arrayfun(@(target) CATnWrapper(arrayfun(@(cell) CATnWrapper(arrayfun(@(model) flip_function(q0_hpf(:,model,cell,target),e1_hpf(:,model,cell,target)),1:n_models,'Un',0),2),1:n_cells,'Un',0),3),1:n_genes,'Un',0),4),[4 1 2 3]);
        Px1_hpf = fliplr(alphas_hpf.*q_hpf);%Probability distribution added in reverse order of coefficients
        Px1_normFactor_hpf =permute(repmat(squeeze(sum(Px1_hpf,2)),[1 1 1 size(Px1_hpf,2)]),[1 4 2 3]);
