...
seqs = cell(size(AccessionNumbers,2),1);%Will store the sequences for all the accession numbers
gene_names = cell(size(AccessionNumbers,2),1);%Will store the gene name for all accession numbers
if (strand==1)
    for i=1:Lh-Ll+1
        Lp = Ll+i-2;
    ...
    for num1 = 1:max(ic1)
        if sum(ic1 == num1) == size(AccessionNumbers,2)+size(TextInclude,2)
            ...
            probes{counter,2} = all_tent_probes{index1};%Store the original probe
