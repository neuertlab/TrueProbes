function [probes,tent_probes,seqs,gene_names,organisms] = BKJH_MultiplexProbe_Generator(L,AccessionNumbers,TextInclude,TextExclude,AccessionExclude,strand,isOffline,SEQdbRoot)
...
%%% This is modified from the regular probe generator to describe the position of each probe within each multiplex RNA.
%%% This is tells if each probe is shared between any number of the multiplex RNA and is useful for designing probes
%%% multiple isoforms.
...
if (strand==1)
                    ...
                    if (~isempty(cObj))
                        ...
                        for k=1:length(BioIFobj)
                            ...
                            if (sum(strcmp(extractBefore(BioKeys{k}{1}(with_dot),'.'),extractBefore(AccessionNumbers{Acc_Num},'.')))>0)
                                isInBioIFobj_loc(k) = with_dot(strcmp(extractBefore(BioKeys{k}{1}(with_dot),'.'),extractBefore(AccessionNumbers{Acc_Num},'.')));
                                isInBioIFobj(k) = 1;
                            end
                        end
                        cObj = find(isInBioIFobj>0,1);
        ...
        if not(isempty(TextInclude))
            for Txt_Num = 1:size(TextInclude,2)
                ...
                for base1 = 1:size(temp_seq,2)-Lp
                    ...
                    all_tent_probes{1,counter} = temp_seq(base1:base1+Lp);
                    ProbePos(counter) = base1;
                    GenePos(counter) = size(AccessionNumbers,2)+Txt_Num;
                    counter = counter+1;
                end
    ...
    %encode info targets
    [~, ~ , ic1] = unique(all_tent_probes,'stable');
    % size(all_tent_probes)
    % size(ic1)
    counter = 1; %Will be used for storing probes
    for num1 = 1:max(ic1)
        PosVector = NaN*ones(1,size(AccessionNumbers,2)+size(TextInclude,2));
        index1 = find(ic1 == num1);
        probes{counter,1} = num1; %Store the location of the original probe
        probes{counter,2} = all_tent_probes{index1(1)};  %Store the original probe
        PosVector(GenePos(index1)) = ProbePos(index1);
        probes{counter,3} = PosVector;%position within each multiplex gene
        counter = counter+1;
    end
        ...
        for exclude1 = 1:size(TextExclude,2)
            ...
            for num1 = 1:size(tentprobes1,1)
                if isempty(strfind(text1,tentprobes1{num1,2}))
                    ...
                    probes{counter,2} = tentprobes1{num1,2};%Store the original probe
        ...
        for exclude1 = 1:size(AccessionExclude,2)
            ...
            if (isOffline)
                ...
                if (~isempty(cObj))
                    ...
                    fprintf("BioKey not found in reference genomes or transcriptomes.")
                    fprintf('\n')
                    fprintf(strcat("Trying to search for matches to ",extractBefore(AccessionNumbers{exclude1},'.')))
                    fprintf('\n')
                    %BioKey version not found
                    isInBioIFobj = zeros(1,length(BioIFobj));
                    isInBioIFobj_loc = zeros(1,length(BioIFobj));
                    for k=1:length(BioIFobj)
                        with_dot = find(contains(BioKeys{k}{:},'.'));
                        if (sum(strcmp(extractBefore(BioKeys{k}{1}(with_dot),'.'),extractBefore(AccessionNumbers{exclude1},'.')))>0)
                            isInBioIFobj_loc(k) = with_dot(strcmp(extractBefore(BioKeys{k}{1}(with_dot),'.'),extractBefore(AccessionNumbers{exclude1},'.')));
                            isInBioIFobj(k) = 1;
                        end
                    end
                    cObj = find(isInBioIFobj>0,1);
    ...
    for num1 = 1:max(ic1)
        ...
        probes{counter,1} = num1; %Store the location of the original probe
        probes{counter,2} = all_tent_probes{index1(1)};  %Store the original probe
        PosVector(GenePos(index1)) = ProbePos(index1);%position within each multiplex gene
        probes{counter,3} = PosVector;
        counter = counter+1;
    end
    probes = sortrows(probes,1);    %Sort by the index in the original sequence
